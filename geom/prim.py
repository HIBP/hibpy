...
class Polygon2D:
    ...
    def to3D(self):  #, normal=None):
        # (n, 2)
        n = self._points.shape[0]
        pts = np.zeros((n, 3))
        pts[:, 0:2] = self._points

        # if normal is not None:
        #     imx = zwardRotateMx(normal)
        #     mx = invMx(imx)
        #     pts = transformPts(pts, mx)
...
class PolygonWithHole3D(Polygon3D):
    def __init__(self, points, hole):
        self._points = np.asarray(points)
        #self._hole = hole_pts
        if hasattr(hole, 'intersect_with_segment'):
            self._hole = hole
        else:
            self._hole = Polygon3D(hole) # hole_pts
        self._center = np.average(self._points, axis=0)
        self._normal = self.calc_normal()
        self._gabarits = None
        self.recalc_gabarits()
    ...
    def plot(self, axes_code=None, *args, **kwargs):
        ...
        self._hole.plot(axes_code=None, *args, **kwargs)
